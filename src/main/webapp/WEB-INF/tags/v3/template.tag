...
<link rel="stylesheet" href="<spring:url value="/css/v3/style.css"/>" />
					...
					<li>
						...
						<ul>
							<c:forEach items="${e.value}" var="a">
								<li><a href=<spring:url value="/v3/select/${a.id}"/>>${a.name}</a></li>
							</c:forEach>
						</ul>
					</li>
				</c:forEach>
				<li>
					<a href='<spring:url value="/v3/army/create"/>'><i class="fas fa-plus-circle"></i> New</a>
				</li>
			</ul>
		</nav>
	</header>

...
public enum GeneriqueRule implements IRule<GeneriqueRule> {
	...
	public String getDescription() throws IOException {
		if (is(ArmyRuleType.AptitudesDeCommandement)) {
			return new DescriptionReader().read("Generique/AptitudesDeCommandement/", getName());
		}
		if (is(ArmyRuleType.ActionsHeroiques)) {
			return new DescriptionReader().read("Generique/ActionsHeroiques/", getName());
		}
		return new DescriptionReader().read("Generique/", getName());
	}
